...
def validate_token(encoded_token):
    ...
    user_id_token = jwt.decode(encoded_token, public_key, audience='bjorneparkappen')
    ...
    if user_id_token.get('aud') != "bjorneparkappen":
        # Return 401 Unauthorized
        raise endpoints.UnauthorizedException(
    ...
    firebase = user_id_token.get('firebase')
    ...
    firebase_identities = firebase.get('identities')

    if firebase_identities is None:
        # Return 401 Unauthorized
        raise endpoints.UnauthorizedException(
            'Credentials not recognised')

    firebase_emails = firebase_identities.get('email')

    if firebase_emails is None or len(firebase_emails) is 0:
        # Return 401 Unauthorized
        raise endpoints.UnauthorizedException(
            'Credentials not recognised')

    email = firebase_emails[0]

    # If user not Bjørneparken staff
    if not (email.endswith("@bjorneparken.no") or email == "callum.veale@gmail.com"):
            raise endpoints.UnauthorizedException(email + " is not a member of Bjorneparken staff.")
